...
if um.property.apartments.script ~= 'ps' then return end
...
lib.callback.register('getApartments', function(source)
    ...
    local apartments = MySQL.query.await('SELECT * FROM properties WHERE owner_citizenid = ? AND apartment IS NOT NULL',
        { citizenid })
