...
local function TakeOutImpound(vehicle)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            TriggerEvent("vehiclekeys:client:SetOwner", tostring(vehicle.plate))
...
local function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result, plate)
            ...
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                ...
                if Config.Inventory ~= 'new-qb-inventory' then
                    TriggerServerEvent('police:server:addTrunkItems', tostring(plate), Config.CarItems)
                end
                SetVehicleEngineOn(veh, true, true)
        ...
        end,vehicleInfo.price, vehicleInfo.vehicle)
    end
end
...
local function HeliGarage(currentSelection)
    local heliMenu = {}
    local authorizedHelis = Config.PoliceHelicopters[currentSelection]
    for veh, data in pairs(authorizedHelis) do
        if Heli and next(Heli) then
            for l, b in pairs(Heli) do
                if b.model == veh then
                    for _,v in pairs(data.ranks) do
                        if v == PlayerJob.grade.level then
                            heliMenu[#heliMenu+1] = {
                                ...
                                opticon = "",
                                optparams = {
                                    event = "police:client:HeliSubMenu",
                                    args = {
                                        vehicle = veh,
                                        vehlabel = data.label,
                                        currentSelection = currentSelection,
    ...
    local header = {
        disabled = true,
        header = Lang:t('menu.garage_title'),
        headerid = 'police_heli_menu', -- unique
        desc = '',
        icon = "helicopter"
    }
    ContextSystem.Open(header, heliMenu)
end
...
RegisterNetEvent('police:client:VehicleSubMenu', function(data)
    ...
    if data.out then
        ...
        SubMenu[#SubMenu+1] = {opthead = Lang:t('menu.takeout', {vehicle = vehLabel}), optdesc = Lang:t('menu.takeoutfor', {price = data.price}), opticon = 'car', optparams = {event = "police:client:TakeOutVehicle", args = {vehicle = data.vehicle, currentSelection = data.currentSelection, livery = data.livery, price = data.price}}}
    end
    local newHeader = {
        disabled = true,
...
RegisterNetEvent('police:client:HeliSubMenu', function(data)
    local vehLabel = data.vehlabel
    local SubMenu = {}
    if data.out then
        SubMenu[#SubMenu+1] = {opthead = Lang:t('menu.returncar', {vehicle = vehLabel}), optdesc = "", opticon = 'car', optparams = {event = "police:client:ReturnVehicle", args = {car = data.car}}}
        table.remove(Heli, data.tableid)
    else
        SubMenu[#SubMenu+1] = {opthead = Lang:t('menu.takeout', {vehicle = vehLabel}), optdesc = Lang:t('menu.takeoutfor', {price = data.price}), opticon = 'helicopter', optparams = {event = "qb-police:client:spawnHelicopter", args = {vehicle = data.vehicle, currentSelection = data.currentSelection, livery = data.livery, price = data.price}}}
    end
    local newHeader = {
        disabled = true,
        header = Lang:t('menu.vehmenu', {vehicle = vehLabel}),
        headerid = 'police_heli_submenu', -- unique
        desc = Lang:t('menu.vehmenudesc'),
        icon = "retweet"
    }
    ContextSystem.Open(newHeader, SubMenu)
...
RegisterNetEvent('qb-police:client:spawnHelicopter', function(data)
    local coords = Config.Locations["helispawn"][data.currentSelection]
    if coords then
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 2.0) then
            QBCore.Functions.Notify(Lang:t("error.clearspawnpoint"), "error", 4500)
            return
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result)
            ...
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                ...
                exports[Config.FuelScript]:SetFuel(veh, 100.0)
                closeMenuFull()
                Heli[#Heli+1] = {veh = veh, model = data.vehicle}
...
local function TakeVehicleMenu()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and PlayerJob.type == "leo" then
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                if IsPedInAnyHeli(PlayerPedId()) then
                    -- DrawText.Close()
                    PolDrawText.Open(Lang:t('info.store_heli'), Config.DrawText.position)
                    heli()
                else
                    PolDrawText.Open(Lang:t('info.take_heli'), Config.DrawText.position)
                    heli(zone.data.zone)
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty and IsPedInAnyVehicle(PlayerPedId(), false) then
                PolDrawText.Open(Lang:t('info.impound_veh'), Config.DrawText.position)
                impound()
            else
                ...
                PolDrawText.Open(Lang:t('menu.pol_impound'), Config.DrawText.position)
